...
| EXPECTED OUTCOME | BEKLENEN CIKTI |
| Name/IP address             Mode KeyID Type KLen Last Atmp  NAK Cook CLen<br>====================================================<br>time.cloudflare.com          NTS     1   30  128  32m    0    0    8   64<br>nts.netnod.se                NTS     1   15  256  751    0    0    8  100<br>ptbtime1.ptb.de              NTS     1   15  256  32m    0    0    8  100<br>time.cloudflare.com          NTS     1   30  128  24m    0    0    8   64<br>nts.netnod.se                NTS     1   15  256  650    0    0    8  100 | Name/IP address             Mode KeyID Type KLen Last Atmp  NAK Cook CLen<br>===================================================<br>time.cloudflare.com          NTS     1   30  128  32m    0    0    8   64<br>nts.netnod.se                NTS     1   15  256  751    0    0    8  100<br>ptbtime1.ptb.de              NTS     1   15  256  32m    0    0    8  100<br>time.cloudflare.com          NTS     1   30  128  24m    0    0    8   64<br>nts.netnod.se                NTS     1   15  256  650    0    0    8  100 |
| IF DOESN'T WORK | EGER CALISMAZSA |
| # 1. Save the current configuration<br>`sudo cp /etc/chrony.conf /etc/chrony.conf.bak`<br><br># 2. Disable the original pool of Fedora (pasif yap)<br>`sudo sed -i '/pool 2.fedora.pool.ntp.org iburst/s/^/#/' /etc/chrony.conf`<br><br># 3. Add ntfs configuration at the end of the file<br>`cat << 'EOF' \| sudo tee -a /etc/chrony.conf`<br><br>`# V30.2_AEGIS: NTS Mühürleme Protokolü`<br>`server time.cloudflare.com iburst nts`<br>`server nts.netnod.se iburst nts`<br>`server ptbtime1.ptb.de iburst nts`<br>`ntsdumpdir /var/lib/chrony`<br>`EOF`<br><br># 4. Erase the old cookies and restart the service<br>`sudo systemctl stop chronyd`<br>`sudo rm -rf /var/lib/chrony/ntscookies/*`<br>`sudo systemctl start chronyd`<br># 5. 10 saniye bekleme (Handshake Süreci)<br>`sleep 10`<br># 6. check the cryptographic seal<br>`sudo chronyc -N authdata` | # 1. Mevcut ana konfigürasyonu atomik olarak yedekle<br>`sudo cp /etc/chrony.conf /etc/chrony.conf.bak`<br><br># 2. Fedora'nın orijinal pool havuzunu disable et (pasif yap)<br>`sudo sed -i '/pool 2.fedora.pool.ntp.org iburst/s/^/#/' /etc/chrony.conf`<br><br># 3. NTS yapılandırmasını dosyanın sonuna doğrudan enjekte et<br>`cat << 'EOF' \| sudo tee -a /etc/chrony.conf`<br><br>`# V30.2_AEGIS: NTS Mühürleme Protokolü`<br>`server time.cloudflare.com iburst nts`<br>`server nts.netnod.se iburst nts`<br>`server ptbtime1.ptb.de iburst nts`<br>`ntsdumpdir /var/lib/chrony`<br>`EOF`<br><br># 4. Eski cookie'leri temizle ve servisi mutlak bir kararlılıkla başlat<br>`sudo systemctl stop chronyd`<br>`sudo rm -rf /var/lib/chrony/ntscookies/*`<br>`sudo systemctl start chronyd`<br># 5. 10 saniye bekleme (Handshake Süreci)<br>`sleep 10`<br># 6. Kriptografik mühürün mühürlenip mühürlenmediğini root olarak doğrula<br>`sudo chronyc -N authdata` |
